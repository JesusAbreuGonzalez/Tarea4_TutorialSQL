/*Ejercicios del curso de SQL en 2 semanas, si desea ejecutar algún ejercicio
  por favor comente los demás ejercicios para evitar errores*/
...
--2.2
	...
	exec sp_tables @table_owner='dbo'
	exec sp_tables @table_owner='libros'
...
--3.2
	...
	exec sp_tables @table_name = 'dbo'
	exec sp_columns @table_name = 'libros'
...
--4.1
	...
	insert into peliculas(nombre, actor, duracion,cantidad)
		values('Mision imposible', 'Tom Cruise', 125, 3)
	insert into peliculas(nombre, actor, duracion, cantidad)
...
--4.2
	...
	exec sp_columns @table_name = 'empleados'
...
--6.1
	...
	insert into agenda (apellido, nombre, domicilio, telefono)
		values ('Bustamente','Betina','Avellaneda 135','4458787');
	insert into agenda (apellido, nombre, domicilio, telefono)
...
--7.1
	...
	exec sp_columns @table_name = 'articulos'
...
--11.1
	...
	exec sp_columns @table_name = 'medicamentos'

	insert into medicamentos(codigo, nombre, laboratorio, precio, cantidad)
		values(1, 'Sertal gotas', null, null, 100)
	insert into medicamentos(codigo, nombre, laboratorio, precio, cantidad)
		values(2, 'Sertal compuestos', null, 8.90, 150)
	insert into medicamentos(codigo, nombre, laboratorio, precio, cantidad)
...
--14.2
	if object_id('peliculas') is not null
	 ...
	 insert into peliculas (codigo,titulo,actor,duracion)
		values(15,'Mision imposible 2','Tom Cruise',120)

	 insert into peliculas (codigo, titulo,actor,duracion)
		values(60,'La vida es bella','zzz',220)
...
--19 - Tipo de dato (fecha y hora)
	if object_id('alumnos') is not null
	 ...
	 insert into alumnos
		values('Perez','Laura','26666666','Bulnes 345','03-03-1991',null)
	 insert into alumnos
		values('Lopez','Carlos','27777777','Sarmiento 1254','15-03-1990',null)

	 select * from alumnos where fechaingreso<'1-1-1991'
...
--27 - Funciones para el uso de fechas y horas
	if object_id ('empleados') is not null
	 ...
	 select documento,
		stuff(ceiling(sueldo),1,0,'$ ')
		from empleados;

	 select nombre,apellido
		from empleados where datename(month,fechanacimiento) = 'october'

	 select nombre,apellido
		from empleados where datepart(year,fechaingreso) = 2000
...
--33 - Búsqueda de patrones (like - not like)
	...
	select * from empleados	where domicilio like 'Co%8%' --like con operador and

	select * from empleados	where documento like '%[02468]' --like con operador or

	select * from empleados	where documento like '[^13]%' and nombre like '%ez' --like con negacion

	select nombre from empleados where nombre like '%[yj]%'

	select nombre,seccion from empleados where seccion like '[SG]_______' --like con condicion de cantidad de caracteres
	...
	-- el % va al final para evaluar como comienza un campo y al comiendo para evaluar el final
...
--37.1
	...
	select ciudad, avg(montocompra) as 'Promedio de compra' from visitantes
		group by ciudad

	select ciudad, count(*) as 'Con mail' from visitantes where mail is not null and mail != 'no tiene'
		group by ciudad
...
--37.2
	...
	select seccion, avg(sueldo) as 'promedio de sueldo' from empleados where cantidadhijos>0 and cantidadhijos is not null
		group by seccion;
...
--38.1
	...
	select ciudad, count(*) as 'Cantidad' from clientes where domicilio like'%San Martin%'
...
--38.2
	...
	select ciudad, sexo, sum(montocompra) as 'total' from visitantes where montocompra>50 and telefono is not null
		group by all ciudad,sexo
		having ciudad != 'Cordoba'
		order by ciudad

	select ciudad,max(montocompra) as maximo from visitantes where domicilio is not null and sexo='f'
		group by all ciudad
...
--43.1
	...
	select count(distinct provincia) as 'cantidad provincias' from clientes

	select distinct ciudad from clientes

	select count(distinct ciudad) as 'cantidad ciudades' from clientes

	select distinct ciudad from clientes where provincia = 'Cordoba'

	select provincia, count(ciudad) as 'Cantidad ciudades' from clientes
		group by provincia
...
--44 - Cláusula top
	...
	insert into empleados
	values ('22222222','Alberto Lopez','c','Sistemas') --c = casado
	insert into empleados
	values ('23333333','Beatriz Garcia','c','Administracion')
	insert into empleados
	values ('24444444','Carlos Fuentes','s','Administracion')--s = soltero
	insert into empleados
...
--53 - Eliminar restricciones (alter table - drop)
	...
	exec sp_helpconstraint vehiculos --Ver restricciones
...
--66.1
	...
	select c.nombre,domicilio,ciudad from clientes as c
		join provincias as p on c.codigoprovincia = p.codigo where p.nombre = 'Santa Fe'
...
--66.2
	...
	insert into inscriptos
		values('Juan Perez','22222222','tenis','s') -- s = paga
	insert into inscriptos 
		values('Maria Lopez','23333333','tenis','s')
	insert into inscriptos 
		values('Agustin Juarez','24444444','tenis','n') -- n = impaga
	insert into inscriptos 
		values('Marta Garcia','25555555','natacion','s')
	insert into inscriptos 
		values('Juan Perez','22222222','natacion','s')
	insert into inscriptos 
		values('Maria Lopez','23333333','natacion','n')

	insert into inasistencias
		values('22222222','tenis','2006-12-01')
	insert into inasistencias
		values('22222222','tenis','2006-12-08')
	insert into inasistencias
		values('23333333','tenis','2006-12-01')
	insert into inasistencias
		values('24444444','tenis','2006-12-08')
	insert into inasistencias
		values('22222222','natacion','2006-12-02')
	insert into inasistencias
		values('23333333','natacion','2006-12-02')

	select nombre,ins.deporte,ina.fecha from inscriptos as ins
...
--70.1
	...
	insert into guardias 
		values('24333444','Alberto Torres','m','San Martin 567')
	insert into guardias 
		values('25333444','Luis Ferreyra','m','Chacabuco 235')
	insert into guardias 
		values('23333444','Lorena Viale','f','Sarmiento 988')
	insert into guardias 
		values('26333444','Irma Gonzalez','f','Mariano Moreno 111')
...
--82 - Unión
	...
	insert into proveedores 
		values('Carnes Unica','Caseros 222')
	insert into proveedores 
		values('Lacteos Blanca','San Martin 987')
	insert into clientes 
		values('Supermercado Lopez','Avellaneda 34')
	insert into clientes 
		values('Almacen Anita','Colon 987')
	insert into clientes 
		values('Garcia Juan','Sucre 345')
	insert into empleados 
...
--92 - Subconsultas como expresión
	...
	select alumnos.*, (select avg(nota) from alumnos) - nota as diferencia from alumnos
...
--93 - Subconsultas con in
	...
	create table clientes(
	 ...
	 primary key(codigo),
	 constraint FK_clientes_ciudad foreign key (codigociudad) references ciudades(codigo)
	 on update cascade,
	);
...
--94 - Subconsultas any - some - all
	...
	insert into socios 
		values('24444444','Carlos Conte','Sarmiento 755')
	insert into socios 
		values('25555555','Fabian Fuentes','Caseros 987')
	insert into socios 
		values('26666666','Hector Lopez','Sucre 344')
	...
	select deporte from inscriptos as i
		where numerosocio=1 and	deporte= any (select deporte from inscriptos as i where numerosocio=2)
...
--96 - Subconsulta - Exists y Not Exists
	...
	select nombre from socios as s
		where exists (select *from inscriptos as i where s.numero=i.numerosocio	and i.deporte='natacion')

	select nombre from socios as s
		where not exists (select *from inscriptos as i where s.numero=i.numerosocio and i.deporte='natacion')

	select s.* from socios as s
		where exists (select *from inscriptos as i where s.numero=i.numerosocio and i.cuotas=10)
...
--98 - Subconsulta en lugar de una tabla
	...
	insert into inscriptos 
		values ('22222222',3,'2006','s') --s = paga
	insert into inscriptos 
		values ('23333333',3,'2006','s')
	insert into inscriptos 
		values ('24444444',3,'2006','n') --n = impaga
	insert into inscriptos 
		values ('22222222',3,'2005','s')
	insert into inscriptos 
		values ('22222222',3,'2007','n')
